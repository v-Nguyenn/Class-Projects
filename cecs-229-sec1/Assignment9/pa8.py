...
class Matrix:
  ...
  def copy(self):
    """Returns a distinct copy of the matrix that will be used to perform operations on.
    :return: a new Matrix object with all rows copied.
    """
    new_rows = []
    for row in self.rows:
      new_rows.append(row[:])
  ...
  def swap_rows(self, i, j):
    """Swaps row i with row j within the matrix. No value is returned, the matrix itself is updated.
    :param i: the index of the first row to be swapped (1-based).
    :param j: the index of the second row to be swapped (1-based).
    """
    temp = self.rows[i - 1]
    self.rows[i - 1] = self.rows[j - 1]
    self.rows[j - 1] = temp

  def scale_row(self, i, c):
    """Multiplies all elements in row i by the scalar c. No value is returned, the matrix itself is updated.
    :param i: the index of the row (1-based).
    :param c: the scalar to multiply the elements by.
    """
    for j in range(len(self.rows[i - 1])):
      self.rows[i - 1][j] *= c

  def add_row_multiple(self, src, tar, c):
    """Multiplies the source row by the scalar value, adds the result to the target row, 
    and overwrites the target row with that result. No value is returned, the matrix itself is updated.
    :param src: the index of the source row (1-based).
    :param tar: the index of the target row (1-based).
    :param c: the scalar to multiply the elements by.
    """
    for j in range(len(self.rows[tar - 1])):
      self.rows[tar - 1][j] += c * self.rows[src - 1][j]
